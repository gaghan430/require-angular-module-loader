define(function() {
    return {
        ...
        validate: function(config, type) {
            ...
            if (!structure.hasOwnProperty('module') || !structure.module.hasOwnProperty('path') || !structure.hasOwnProperty('prefix')) {
        ...
        validateType: function(structure, type) {
            if (!structure.hasOwnProperty(type) && !structure[type].hasOwnProperty('path')) {
        ...
        getCurrentModule: function(config, url) {
            ...
            return url.replace(new RegExp('^.*' + (baseUrl + prefix.replace(/{module}/g, ''))), '').split('/')[0];
        },
        /**
         *
         * @param path
        ...
        path: function(path, config, url, module) {
            var prefix = config.structure.prefix;
            return prefix.replace(/{module}/g, (module || this.getCurrentModule(config, url))) + path;
        },
        /**
         *
         * @param name
        ...
        reqPath: function(type, name, config, url) {
            ...
            var path = structure[type].path.replace(new RegExp('{' + type + '}', 'g'), component);
